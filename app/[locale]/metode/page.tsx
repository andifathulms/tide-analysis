  ...
  return (
    <div className="space-y-8">
      ...
      <section className="max-w-3xl space-y-3">
        <h1 className="text-2xl">{dict.metode.title}</h1>
        <p>{dict.metode.lead}</p>
      </section>

      <section className="max-w-3xl space-y-3">
        <h2 className="text-xl">Astronomi</h2>
        <p>
      ...
      <section className="max-w-3xl space-y-3">
        <h2 className="text-xl">Pencocokan</h2>
        <p>
      ...
      <section className="space-y-3">
        <h2 className="text-xl">Himpunan komponen</h2>
          ...
          <table className="w-full border-collapse text-sm">
            ...
            <tbody className="numeric">
              {CONSTITUENTS.map((constituent) => (
                <tr key={constituent.name} className="border-b border-grid/50">
                  ...
                  <td className="control py-1.5 text-xs">{constituent.description}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <section className="space-y-3">
        <h2 className="text-xl">Rekaman dan hasilnya</h2>
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="control border-b border-grid text-left text-xs uppercase tracking-wide">
            ...
            <tbody className="numeric">
              {analyses.map((analysis) => {
                ...
                return (
                  <tr key={analysis.station.stationId} className="border-b border-grid/50">
                    <th scope="row" className="py-1.5 pr-4 text-left font-medium">
                      <Link
      ...
      <section className="max-w-3xl space-y-3">
        <h2 className="text-xl">Metode Admiralty</h2>
        <p>
      ...
      <section className="max-w-3xl space-y-3">
        <h2 className="text-xl">Sumber data dan lisensi</h2>
        <ul className="space-y-3 text-sm">
          {MANIFEST.sources
            .filter((source) => source.id !== 'synthetic')
            .map((source) => (
              <li key={source.id} className="card p-3">
                ...
                {source.attribution !== '' && (
                  <p className="mt-1 text-traceInk/70">{source.attribution}</p>
                )}
                <p className="mt-1 text-traceInk/70">{source.note}</p>
                {source.termsUrl !== '' && (
                  <p className="mt-1">
                    <a
